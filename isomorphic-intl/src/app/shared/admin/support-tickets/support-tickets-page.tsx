  ...
  const handleAssignAgent = async (ticketId: string, agentId: string) => {
    ...
    try {
      ...
      setTickets((prev) =>
        prev.map((t) =>
          t.id === ticketId
            ? { ...t, assignedToSupportId: agentId, status: TicketStatus.Assigned }
            : t
        )
      );
  ...
  const filteredTickets = useMemo(() => {
    return tickets.filter((ticket) => {
      ...
      const matchesCategory =
        categoryFilter === 'all' || ticket.category.toString() === categoryFilter;

      // Priority filter (client-side fallback)
      const matchesPriority =
        priorityFilter === 'all' || ticket.priority.toString() === priorityFilter;
                    ...
                    <td className="px-4 py-3">
                      {ticket.status === TicketStatus.Closed ? (
                        ...
                        <Select
                          ...
                          onChange={(option: any) =>
                            handleAssignAgent(ticket.id, option?.value)
                          }
