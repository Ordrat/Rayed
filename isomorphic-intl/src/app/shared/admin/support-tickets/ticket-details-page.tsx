...
import { Title, Text, Badge, Button, Loader, Select } from "rizzui";
import { PiArrowLeftBold, PiChatCircleText, PiCheckCircleBold, PiUserCirclePlus, PiXCircleBold } from "react-icons/pi";
    ...
    <>
      <PageHeader title={pageHeader.title} breadcrumb={pageHeader.breadcrumb} isStaticTitle={true}>
        <div className="flex items-center gap-2">
          ...
          {!isClosed && (
            <Button onClick={handleCloseTicket} isLoading={isClosing} className="bg-green-600 hover:bg-green-700">
          ...
          <div className="rounded-lg border border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-gray-800">
            ...
            <div className="space-y-4">
              <div className="flex justify-between">
                <Text className="text-gray-500">{lang === "ar" ? "الحالة" : "Status"}</Text>
                <Badge
                  color={
              ...
              <div className="flex justify-between">
                <Text className="text-gray-500">{lang === "ar" ? "الفئة" : "Category"}</Text>
                <Text className="font-medium">{TicketCategoryLabels[ticket.category]?.[lang] || "Unknown"}</Text>
              </div>
              <div className="flex justify-between">
                <Text className="text-gray-500">{lang === "ar" ? "الأولوية" : "Priority"}</Text>
                <Badge color={ticket.priority === 4 ? "danger" : ticket.priority === 3 ? "warning" : "info"}>
                  {TicketPriorityLabels[ticket.priority]?.[lang] || "Unknown"}
                </Badge>
              </div>
              <div className="flex justify-between">
                <Text className="text-gray-500">{lang === "ar" ? "تاريخ الإنشاء" : "Created"}</Text>
                <Text className="font-medium">{new Date(ticket.createdAt).toLocaleDateString()}</Text>
              </div>
            </div>
          </div>
                  ...
                  {archivedMessages.length === 0 ? (
                    ...
                    archivedMessages.map((msg) => (
                      ...
                      >
                        ...
                        >
                          {msg.senderType !== SenderType.Support && (
                            <Text className="text-xs font-semibold text-primary mb-1">{msg.senderName}</Text>
                          )}
                          <Text>{msg.messageText}</Text>
                          <Text className="text-xs opacity-70 mt-1">{new Date(msg.sentAt).toLocaleTimeString()}</Text>
                        </div>
                      </div>
                    ))
                  )}
